...
def steghide_extract(carrier,password):
        subprocess.getoutput(["Tools/hide in files/steghide/steghide.exe"
                      ,"extract"
                      ,"-sf"
                      ,carrier
                      ,"-p"
...
def Back():
    root.destroy()
    subprocess.Popen(['python', "hide_in_file_page.py"], cwd=os.path.dirname(os.path.abspath(__file__)))

btn_back=ttk.Button(root,text="back",command=Back )
btn_back.grid(row=10,column=1,padx=10,pady=20)
